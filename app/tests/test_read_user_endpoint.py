...
from app.main import read_user_endpoint
...
"""
def test_read_user_endpoint_001_self_none(mocker, make_auth):
    ...
    response = read_user_endpoint(request, auth, db=mocker.MagicMock())
...
"""
def test_read_user_endpoint_002_self_named(mocker, make_auth):
    auth = make_auth(user_name="login_user")
...
"""
def test_read_user_endpoint_003_other_by_admin(mocker, make_auth, make_user):
...
"""
def test_read_user_endpoint_004_other_by_non_admin(mocker, make_auth):
    auth = make_auth(user_name="login_user")
    request = UserGetRequest(user_name="other_user")

    with pytest.raises(HTTPException) as exc_info:
        read_user_endpoint(request, auth, db=mocker.MagicMock())
...
"""
def test_read_user_endpoint_005_not_found(mocker, make_auth):
    auth = make_auth(user_name="login_user", admin_flag=True)
    request = UserGetRequest(user_name="other_user")
