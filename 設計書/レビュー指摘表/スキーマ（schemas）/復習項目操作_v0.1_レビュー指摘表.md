<table style="border-collapse: collapse;">
    <tr>
        ...
        <th colspan="3" style="border: 3px solid white;">スキーマ（schemas）/復習項目操作</th>
    ...
    <tr>
        ...
        <td style="border: 1px solid white;">スキーマ「復習スケジュール（ReviewSchedule）」の「復習回（review_time）」の型が `str` と定義されているが、テーブル定義書では INTEGER（整数型）、CLUD設計書「復習管理情報作成」でも `int` として扱われている。型は `int` に修正すべきである。なお復習項目作成レスポンス内の同フィールドにも同様の誤りが記載されている。</td>
        <td style="border: 1px solid white;">234</td>
        ...
        <td style="border: 1px solid white;">review_timeの型でstrとなっている箇所をintに修正</td>
        <td style="border: 1px solid white;">「ReviewSchedule」と「ReviewCreateResponse」のreview_timeの型をstr→intに修正</td>
