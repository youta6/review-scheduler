<table style="border-collapse: collapse;">
    <tr>
        ...
        <th colspan="3" style="border: 3px solid white;">CLUD/復習項目操作/復習情報作成</th>
    ...
    <tr>
        ...
        <td style="border: 1px solid white;">復習管理情報の作成（REVIEW_MANAGEMENTへのINSERT）処理が設計に含まれていない。機能要件では「復習情報登録時に5回分の復習スケジュールを自動生成する」とあるが、本設計書は REVIEWテーブルへの登録のみ設計しており、REVIEW_MANAGEMENTテーブルへの登録が含まれていない。サーバー処理（main）側で `create_review_management` を別途呼び出す設計であれば問題ないが、その旨の記載または参照が不足している。</td>
        ...
        <td style="border: 1px solid white;">REVIEWMANAGEMENTへの登録はmainで別のcrudを呼び出して行っている。<br>その旨の記載または参照についてもcrud側が気にする必要はなく、create_reviewの責任範囲外のため、対応不要。</td>
